...
const productsOptions = [
  ...
  { value: 223, label: "Stocked II", remaining: 34, rate: 320 },
];
...
export default function SaleForm({ sale, onClose }) {
  ...
  // form submission for adding or updating sale
  ...
  // side effect to sync selected products with form fields
  useEffect(() => {
    ...
    if (selectedProducts.length < fields.length) {
      fields.forEach((item, index) => {
        if (
          !selectedProducts.some((product) => product.value === item.sku_id)
        ) {
          remove(index);
        }
      });
